...
export const recognizeNumberGame={
  ...
  play(api){
    ...
    const number=min+api.rand(max-min+1);
    ...
    api.setHint(()=>{api.highlightCorrect();api.setBubble(`Petit indice : je cherche ${word}.`);api.say(`Je cherche le nombre ${spoken}.`,true)});
  }
};
