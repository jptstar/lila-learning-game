export const shadowsGame={
  ...
  play(api){
    ...
    const opts=[target,...wrong].map(x=>({key:x.word,label:x.word,speech:x.word,html:`<span>${x.emoji}</span>`,className:"shadowChoice"}));
    ...
    api.setHint(()=>{api.highlightCorrect();api.say(`Cette ombre correspond à ${target.word}.`,true);});
  }
};
