...
const DISCOVERY_MODES=new Set(["ordered","random"]);
...
const savedDiscoveryMode=localStorage.getItem("lilaDiscoveryMode")||"ordered";
...
const state={
  ...
  discoverMode:DISCOVERY_MODES.has(savedDiscoveryMode)?savedDiscoveryMode:"ordered",
...
function resetSeries(){state.seriesAnswered=0;state.seriesCorrect=0;state.rewardShown=false;updateStats();}
function clearStage(){
...
function showReward(){
  if(state.rewardShown||state.seriesAnswered!==10)return;
  state.rewardShown=true;clearAdvanceTimer();els.reward.classList.add("show");
...
function letterSpoken(letter){
  const found=DATA.find(x=>x.l===String(letter).toUpperCase())||LETTER_CATALOG.find(x=>x.l===String(letter).toUpperCase());
  return found?.spoken||String(letter);
}

async function afterAnswer(ok,item,{selectedKey=null,correctKey=null,selectedLabel=null,correctLabel=null,roundId=state.roundId}={}){
  state.questions++;state.seriesAnswered=Math.min(10,state.seriesAnswered+1);clearAdvanceTimer();

  if(ok){
    state.score++;state.streak++;state.seriesCorrect++;els.feedback.textContent="✅ Bravo !";
    els.bubble.textContent="Oui, c’est exactement ça !";burst();updateStats();
    if(state.seriesAnswered===10){showReward();return;}
  ...
  if((type==="count"||type==="recognize-number")&&selectedKey!==null&&correctKey!==null){
    ...
    els.feedback.innerHTML=`<span class="feedbackWrong">❌ Tu as choisi <strong>${selectedKey}</strong>.</span> <span class="feedbackCorrect">✅ La bonne lettre est <strong>${correctKey}</strong>.</span>`;
    els.bubble.textContent=`Tu as choisi ${selectedKey}. La bonne lettre est ${correctKey}.`;
    explanation=`Ce n'est pas la bonne réponse, mais ce n'est pas grave. Tu as sélectionné la lettre ${letterSpoken(selectedKey)}. La bonne réponse est la lettre ${letterSpoken(correctKey)}. La lettre choisie est en rouge et la bonne réponse est en vert.`;
  }else if(type==="article"){
    const phrase=item.correctPhrase||`${correctKey} ${item.word||""}`.trim();
    els.feedback.innerHTML=`<span class="feedbackWrong">❌ Tu as choisi <strong>${selectedKey}</strong>.</span> <span class="feedbackCorrect">✅ On dit <strong>${phrase}</strong>.</span>`;
    els.bubble.textContent=`Tu as choisi « ${selectedKey} ». On dit « ${phrase} ».`;
    explanation=`Ce n'est pas la bonne réponse, mais ce n'est pas grave. Tu as sélectionné ${selectedKey}. On dit ${phrase}. La réponse choisie est en rouge et la bonne réponse est en vert.`;
  }else if(["shape","shadow"].includes(type)){
    const selected=selectedLabel||String(selectedKey),correct=correctLabel||String(correctKey);
    els.feedback.innerHTML=`<span class="feedbackWrong">❌ Tu as choisi <strong>${selected}</strong>.</span> <span class="feedbackCorrect">✅ La bonne réponse est <strong>${correct}</strong>.</span>`;
    els.bubble.textContent=`Tu as choisi ${selected}. La bonne réponse est ${correct}.`;
    explanation=`Ce n'est pas la bonne réponse, mais ce n'est pas grave. Tu as choisi ${selected}. La bonne réponse est ${correct}. La réponse choisie est en rouge et la bonne réponse est en vert.`;
  }else{
  ...
  if(item?.l&&type!=="article")state.mistakeQueue.push(item);
...
function pickLearningItem({slot="letters",avoidLetter=false}={}){
  const last=state.lastTargets[slot];let item=null;
  ...
  state.lastTargets[slot]={word:item.word,letter:item.l};return item;
}
function pickNumber(min,max,slot="numbers"){
  const low=Math.max(0,Math.min(100,Number(min))),high=Math.max(low,Math.min(100,Number(max)));
  const last=state.lastTargets[slot]?.number,values=[];
  for(let n=low;n<=high;n++)if(n!==last)values.push(n);
  const number=values.length?values[rand(values.length)]:low;state.lastTargets[slot]={number};return number;
}
function pickUnique(items,slot,keyFn=x=>x?.id??x?.word??x){
  if(!items.length)return null;
  const previous=state.lastTargets[slot]?.value;
  const candidates=items.filter(item=>String(keyFn(item))!==String(previous));
  const item=(candidates.length?candidates:items)[rand((candidates.length?candidates:items).length)];
  state.lastTargets[slot]={value:keyFn(item)};return item;
}
...
function renderChoices(options,correctKey,{item=null,slotKey=state.gameId}={}){
  ...
  const correctOption=options.find(o=>String(o.key)===String(correctKey));
  arrangeOptions(options,correctKey,slotKey).forEach(opt=>{
    ...
    b.onclick=()=>{
      ...
      const answerRound=state.roundId,ok=String(opt.key)===String(correctKey);b.classList.add(ok?"good":"bad");
      if(state.correctChoice)state.correctChoice.classList.add("good");
      afterAnswer(ok,item,{
        selectedKey:opt.key,correctKey,roundId:answerRound,
        selectedLabel:opt.speech||opt.label||String(opt.key),
        correctLabel:correctOption?.speech||correctOption?.label||String(correctKey)
      });
    };
...
const api={
  ...
  pickLearningItem,pickNumber,pickUnique,
  ...
  setSmartSubgame:g=>state.smartSubgame=g,getGame:id=>GAMES.find(g=>g.id===id),clearChoices:()=>els.choices.innerHTML="",
...
function nextRound(){clearAdvanceTimer();voice.cancel();state.roundId++;els.reward.classList.remove("show");clearStage();const game=currentGame();if(game)game.play(api);}
...
function goHome(){clearAdvanceTimer();state.roundId++;voice.cancel();resetSeries();$("#gameScreen").classList.remove("active");$("#home").classList.add("active");}
function showHint(){
...
$("#homeBtn").onclick=goHome;$("#repeatBtn").onclick=()=>voice.repeat();$("#hintBtn").onclick=showHint;
...
function toggle(id,key,onValue,offValue,onChange){const el=$(id);el.onclick=()=>{el.classList.toggle("on");state[key]=el.classList.contains("on")?onValue:offValue;if(onChange)onChange(state[key]);};}
toggle("#autoSpeakToggle","autoSpeak",true,false,v=>voice.configure({autoSpeak:v}));toggle("#lowerToggle","showLower",true,false);

const discoveryModeSelect=$("#discoveryModeSelect");
if(discoveryModeSelect){
  discoveryModeSelect.value=state.discoverMode;
  discoveryModeSelect.onchange=()=>{state.discoverMode=DISCOVERY_MODES.has(discoveryModeSelect.value)?discoveryModeSelect.value:"ordered";localStorage.setItem("lilaDiscoveryMode",state.discoverMode);};
}

const minNumberSelect=$("#minNumberSelect"),maxNumberSelect=$("#maxNumberSelect");
RANGE_VALUES.forEach(n=>{for(const select of [minNumberSelect,maxNumberSelect]){const option=document.createElement("option");option.value=String(n);option.textContent=String(n);select.appendChild(option);}});
...
function syncRange(changed){
  let min=normalizeBound(minNumberSelect.value,0),max=normalizeBound(maxNumberSelect.value,10);
  if(max<=min){if(changed==="min"){if(min<100)max=min+10;else{min=90;max=100;}}else{if(max>0)min=max-10;else{min=0;max=10;}}}
  state.minNumber=min;state.maxNumber=max;minNumberSelect.value=String(min);maxNumberSelect.value=String(max);
  localStorage.setItem("lilaMinNumber",String(min));localStorage.setItem("lilaMaxNumber",String(max));refreshRangeOptions();
}
minNumberSelect.value=String(state.minNumber);maxNumberSelect.value=String(state.maxNumber);minNumberSelect.onchange=()=>syncRange("min");maxNumberSelect.onchange=()=>syncRange("max");syncRange("max");

const voiceProviderSelect=$("#voiceProviderSelect"),piperSettings=$("#piperSettings"),piperEndpointInput=$("#piperEndpointInput"),piperTokenInput=$("#piperTokenInput"),piperStatus=$("#piperStatus"),testPiperBtn=$("#testPiperBtn");
function setPiperStatus(text,kind=""){piperStatus.textContent=text;piperStatus.className=`piperStatus ${kind}`.trim();}
function refreshPiperPanel(){const enabled=state.voiceProvider==="piper";piperSettings.hidden=!enabled;if(enabled&&!state.piperEndpoint)setPiperStatus("Indique l’adresse HTTPS de Piper.","warn");else setPiperStatus("");}
function saveVoiceSettings(){
  ...
  localStorage.setItem("lilaVoiceProvider",state.voiceProvider);localStorage.setItem("lilaPiperEndpoint",state.piperEndpoint);localStorage.setItem("lilaPiperToken",state.piperToken);
  voice.configure({provider:state.voiceProvider,piperEndpoint:state.piperEndpoint,piperToken:state.piperToken});refreshPiperPanel();
}
voiceProviderSelect.value=state.voiceProvider;piperEndpointInput.value=state.piperEndpoint;piperTokenInput.value=state.piperToken;
voiceProviderSelect.onchange=saveVoiceSettings;piperEndpointInput.onchange=saveVoiceSettings;piperTokenInput.onchange=saveVoiceSettings;
testPiperBtn.onclick=async()=>{
  saveVoiceSettings();if(!state.piperEndpoint){setPiperStatus("Adresse Piper manquante.","error");return;}
  testPiperBtn.disabled=true;setPiperStatus("Connexion à Piper…");
  try{await voice.testPiper("Bonjour ! Je suis Lila.");setPiperStatus("✅ Piper fonctionne.","ok");}catch(e){console.warn(e);setPiperStatus("❌ Piper est inaccessible.","error");}finally{testPiperBtn.disabled=false;}
};
...
refreshPiperPanel();renderHome();updateStats();window.LilaApp={state,voice};
